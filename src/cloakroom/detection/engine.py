...
HEBREW_BACKEND_ENV = "CLOAKROOM_HEBREW_NLP_ENGINE"
HEBREW_STANZA_MODEL_ENV = "CLOAKROOM_HEBREW_STANZA_MODEL"
HEBREW_TRANSFORMER_MODEL_ENV = "CLOAKROOM_HEBREW_TRANSFORMER_MODEL"
HEBREW_TRANSFORMER_SPACY_MODEL_ENV = "CLOAKROOM_HEBREW_TRANSFORMER_SPACY_MODEL"
...
def _resolve_hebrew_transformer_spacy_model() -> str:
    preferred = _env_value(
        HEBREW_TRANSFORMER_SPACY_MODEL_ENV,
        default="",
    ).strip()
    if preferred:
...
class DetectionEngine:
    ...
    def __init__(
        ...
        self._hebrew_backend = _normalize_hebrew_backend(
            ...
            or _env_value(
                HEBREW_BACKEND_ENV,
                default=DEFAULT_HEBREW_BACKEND,
            )
        )
        self._hebrew_stanza_model = (
            hebrew_stanza_model
            or _env_value(
                HEBREW_STANZA_MODEL_ENV,
                default=DEFAULT_HEBREW_STANZA_MODEL,
            )
        ).strip()
        self._hebrew_transformer_model = (
            hebrew_transformer_model
            or _env_value(
                HEBREW_TRANSFORMER_MODEL_ENV,
                default=DEFAULT_HEBREW_TRANSFORMER_MODEL,
            )
        ).strip()
