...
from cloakroom.handlers.column_select import parse_columns_option
from cloakroom.ipc.server import IPCServer
from cloakroom.ipc.stdio_server import main as stdio_server_main
from cloakroom.logging import (
    append_audit_event,
    collect_log_payload,
    configure_logging,
    delete_audit_events,
...
from cloakroom.exceptions import CloakroomError
from cloakroom.exceptions import LicenseFeatureError
from cloakroom.exceptions import LicenseKeyInvalidError
from cloakroom.exceptions import LicenseLimitExceededError
from cloakroom.exceptions import WorkspaceNotFoundError
...
from cloakroom.performance import run_csv_clipboard_benchmark, write_benchmark_result_json
...
console = Console()
ONBOARDING_MARKER = Path.home() / ".cloakroom" / ".onboarding_complete"
...
def _get_or_create_workspace_with_warning(
    ...
    *,
    ...
    if not existed:
        ...
        console.print(
            f"  [dim]cloakroom workspace export-key --workspace {name} "
            f"--output ~/.cloakroom/recovery/{name}.recovery.key[/]"
        )
        log_event(
            "cli",
            py_logging.WARNING,
...
@click.version_option(package_name="cloakroom")
@click.pass_context
def main(ctx, verbose, no_logging, encrypt_logs):
    """Cloakroom -- Reversible document anonymization for safe LLM usage."""
    configure_logging(
        component="cli",
        verbose=verbose,
...
@click.option(
    ...
    "output_path",
    default="",
    help="Recovery key output path (default: ~/.cloakroom/recovery/<workspace>.recovery.key).",
)
@click.option(
    "--passphrase",
    default=None,
    ...
    try:
        ...
        if export_key:
            destination = (
                ...
                else Path.home() / ".cloakroom" / "recovery" / f"{workspace}.recovery.key"
            )
...
def logs_export(output_path, workspace_name, include_app, include_audit):
    ...
    try:
        ...
        if output_path:
            ...
            destination = Path.home() / ".cloakroom" / "logs" / f"support-export-{timestamp}.json"
...
def anonymize(
    ...
    \b
    Examples:
        cloakroom anonymize report.xlsx
        cloakroom anonymize data.csv -w client-acme
        cloakroom anonymize contract.docx -o contract.safe.docx
        cloakroom anonymize notes.txt --language he
        cloakroom anonymize intake.pdf --pdf-output-format md
        cloakroom anonymize deals.xlsx --columns "Deal ID,Client Name"
        cloakroom anonymize deals.csv --columns A,C --detect-pii
    """
    try:
        log_event(
            "cli",
...
def restore(file, output, workspace, license_key):
    ...
    \b
    Examples:
        cloakroom restore report.anonymized.xlsx
        cloakroom restore data.csv -w client-acme
        cloakroom restore intake.anonymized.md -w client-acme
    """
    try:
        log_event(
            "cli",
...
@click.option(
    ...
    "output_path",
    type=click.Path(),
    default=str(Path.home() / ".cloakroom" / "performance" / "latest.json"),
    show_default=True,
...
def shield_clipboard_cmd(
    ...
    Example:
        cloakroom shield-clipboard -w client-a --language he
    """
    try:
        log_event(
            "cli",
...
@click.option(
    "--socket-path",
    type=click.Path(),
    default=str(Path.home() / ".cloakroom" / "ipc" / "engine.sock"),
    show_default=True,
