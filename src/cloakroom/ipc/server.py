...
from cloakroom.ipc.framing import recv_frame, send_frame
...
from cloakroom.licensing import FREE_MAX_TTL_HOURS, enforce_license_policy, resolve_license_context
...
from cloakroom.vault.keychain import get_master_key, store_master_key
from cloakroom.vault.recovery import (
    export_encrypted_master_key,
    import_encrypted_master_key,
)
from cloakroom.workspace.manager import WorkspaceContext, WorkspaceManager


class WorkspaceSyncError(CloakroomError):
...
class IPCServer:
    ...
    def _require_str(payload: dict[str, Any], key: str) -> str:
        ...
        if not isinstance(value, str) or not value.strip():
            raise CloakroomError(f"'{key}' must be a non-empty string")
        return value.strip()
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cloakroom IPC socket server")
    parser.add_argument(
        "--socket-path",
