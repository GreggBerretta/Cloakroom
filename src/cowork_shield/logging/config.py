"""Structured logging configuration for CoWork Shield."""
...
from cowork_shield.logging.sanitizer import sanitize_exception, sanitize_text, sanitize_value

LOG_DIR = Path.home() / ".cowork_shield" / "logs"
LOG_FILE = LOG_DIR / "cowork_shield.log"
...
def configure_logging(
    *,
    ...
    logger = logging.getLogger("cowork_shield")
...
def get_logger(component: str) -> logging.Logger:
    return logging.getLogger(f"cowork_shield.{component}")
...
def list_log_files() -> list[Path]:
    ...
    return sorted(
        path
        for path in LOG_DIR.glob("cowork_shield.log*")
        if path.is_file()
    )
...
def delete_log_files() -> int:
    """Delete all application log files under ~/.cowork_shield/logs."""
    count = 0
    for path in list_log_files():
        try:
            path.unlink()
